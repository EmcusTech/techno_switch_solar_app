...
//   int _enHighPower = 0; // 0 or 1
//   int _paLevel = 0; // 0 to 7
//   double? _txPowerLevelDbm; // Calculated TX Power Level in dBm
...
//   // ACK/NACK waiting state
//   Completer<bool?>? _ackNackCompleter;
//   int? _waitingForCommand;
...
//     final connectedDevice = await _btUtils.getConnectedDevice();
...
//       // Check if we're waiting for ACK/NACK response
//       if (_ackNackCompleter != null && !_ackNackCompleter!.isCompleted) {
//         // Check payload for ACK/NACK status
//         if (frame.payloadData.isNotEmpty) {
//           final String status = frame.payloadData.first;
//           Logger('ACK/NACK - Status received in handshake handler: $status');

//           if (status == _ack) {
//             Logger('ACK/NACK - ACK received (0x02)');
//             _ackNackCompleter!.complete(true);
//             return; // Don't process further
//           } else if (status == _nack) {
//             Logger('ACK/NACK - NACK received (0x03)');
//             _ackNackCompleter!.complete(false);
//             return; // Don't process further
//           }
//         }
//       }
...
//           // ACK/NACK handling is done above in the handshake handler
...
//   /// Calculates TX Power Level (dBm) based on EN_HIGH_POWER and PA_LEVEL
//   /// Returns null if invalid combination
//   double? _calculateTxPowerLevel(int enHighPower, int paLevel) {
//     if (paLevel < 0 || paLevel > 7) return null;
//     if (enHighPower != 0 && enHighPower != 1) return null;

//     // Table mapping based on the image
//     if (enHighPower == 0) {
//       // EN_HIGH_POWER = 0
//       switch (paLevel) {
//         case 0:
...
//   /// Creates a BLE frame for TX Power Level command (0x1006) with EN_HIGH_POWER and PA_LEVEL in payload
//   Future<Uint8List> _createTxPowerLevelFrame({
//     required int enHighPower,
//     required int paLevel,
//   }) async {
//     // Payload: [EN_HIGH_POWER (1 byte), PA_LEVEL (1 byte)]
//     List<int> payload = [enHighPower, paLevel];
//     int payloadSize = payload.length;
//     int command = 0x1006; // BLE_PANEL_DOWNLOAD_CONFIG_FROM_BLE_CMD
...
//     int calculatedCRC = convertCrc16(newBleFrame);
//     Logger(
//       'TX Power Level Frame - Calculated CRC: $calculatedCRC (0x${calculatedCRC.toRadixString(16).toUpperCase().padLeft(4, '0')})',
//     );
...
//     Logger(
//       'TX Power Level Frame - Complete frame (${completeBleFrame.length} bytes) before encryption',
//     );
...
//     Logger(
//       'TX Power Level Frame - Encrypted frame (${encryptedDataPacket.length} bytes) ready for transmission',
//     );
...
//       Logger('ACK/NACK - Device disconnected');
//       return null;
//     }

//     // Use existing notification listener instead of creating new one
//     _ackNackCompleter = Completer<bool?>();
//     _waitingForCommand = expectedCommand;

//     try {
//       // Wait for response with timeout
//       final result = await _ackNackCompleter!.future.timeout(
//         timeout,
//         onTimeout: () {
//           Logger('ACK/NACK - Timeout waiting for response');
//           _ackNackCompleter = null;
//           _waitingForCommand = null;
//           return null;
//         },
//       );
...
//   /// Sends the TX Power Level command (0x1006) with EN_HIGH_POWER and PA_LEVEL
//   Future<void> _sendTxPowerLevelCommand() async {
...
//     Logger('TX Power Level Command - Sending TX Power Level command');
//     final connectedDevice = await _btUtils.getConnectedDevices();
//     if (connectedDevice == null) {
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(
...
//     setState(() {
//       _isSending = true;
//       _txPowerLevelDbm = null; // Reset before sending
//     });

//     try {
//       // Create TX Power Level frame with EN_HIGH_POWER and PA_LEVEL
//       final frame = await _createTxPowerLevelFrame(
//         enHighPower: _enHighPower,
//         paLevel: _paLevel,
//       );

//       Logger('TX Power Level Command - Frame created: ${frame.toList()}');
...
//           writeSuccess = success;
//           if (success) {
//             Logger('TX Power Level Command - Command sent successfully');
//           } else {
//             Logger('TX Power Level Command - Failed to send command');
//           }
//         },
//       );
...
//       if (writeSuccess) {
//         // Wait for ACK/NACK response
//         Logger('TX Power Level Command - Waiting for ACK/NACK...');
//         final ackResult = await _waitForAckNack(
//           expectedCommand: 0x1006,
//           timeout: const Duration(seconds: 5),
//         );
...
//           if (ackResult == true) {
//             // Calculate and display TX Power Level
//             final txPower = _calculateTxPowerLevel(_enHighPower, _paLevel);
//             setState(() {
//               _txPowerLevelDbm = txPower;
//             });

//             message = 'TX Power Level set successfully - ACK received';
//             backgroundColor = Colors.green;
//             Logger(
//               'TX Power Level Command - ACK received - Command successful. TX Power Level: ${txPower?.toStringAsFixed(1)} dBm',
//             );
//           } else if (ackResult == false) {
//             message = 'TX Power Level command failed - NACK received';
//             backgroundColor = Colors.orange;
//             Logger('TX Power Level Command - NACK received - Command failed');
//           } else {
//             message = 'TX Power Level command sent - No response received';
//             backgroundColor = Colors.blue;
//             Logger(
//               'TX Power Level Command - No response received (timeout or error)',
//             );
//           }
...
//           ScaffoldMessenger.of(context).showSnackBar(
//             const SnackBar(
//               content: Text('Failed to send TX Power Level command'),
//               backgroundColor: Colors.red,
//             ),
//           );
//         }
...
//       Logger('TX Power Level Command - Frame sent to device');
//     } catch (e) {
//       Logger('TX Power Level Command - Error: $e');
//       if (mounted) {
//         setState(() {
//           _isSending = false;
//         });
...
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(
//             content: Text('Error sending TX Power Level command: $e'),
//             backgroundColor: Colors.red,
//           ),
//         );
//       }
...
//                   ),
//                   SizedBox(height: 32),
//                   // TX Power Level Configuration Card
//                   Container(
//                     padding: EdgeInsets.all(16),
//                     decoration: BoxDecoration(
//                       color: Colors.white,
...
//                                               if (newValue != null) {
//                                                 setState(() {
//                                                   _enHighPower = newValue;
//                                                   _txPowerLevelDbm =
//                                                       null; // Reset when changed
//                                                 });
//                                               }
//                                             },
//                                   ),
...
//                                     ),
//                                     items:
//                                         List.generate(8, (index) => index).map((
//                                           int value,
//                                         ) {
//                                           return DropdownMenuItem<int>(
//                                             value: value,
...
//                         ),
//                         SizedBox(height: 16),
//                         // TX Power Level Display
//                         if (_txPowerLevelDbm != null)
//                           Container(
...
//                                 ),
//                                 Text(
//                                   '${_txPowerLevelDbm!.toStringAsFixed(1)} dBm',
//                                   style: GoogleFonts.inter(
//                                     fontSize: 16,
//                                     fontWeight: FontWeight.w700,
//                                     color: Color(0xFF4CAF50),
