...
import 'package:techno_switch_solar_app/screens/desktop/desktop_scanned_screen.dart';
...
enum DesktopScanType { usb, bluetooth }

class DesktopScanningScreen extends StatefulWidget {
  final bool? isLiveEvent;
  const DesktopScanningScreen({super.key, this.isLiveEvent = false});
...
class _DesktopScanningScreenState extends State<DesktopScanningScreen>
  ...
  DesktopScanType? _selectedScanType;
  ...
  void initState() {
    ...
    _startScanning(DesktopScanType.bluetooth);
  }
  ...
  Future<void> _startScanning(DesktopScanType scanType) async {
    setState(() {
    ...
    if (scanType == DesktopScanType.bluetooth) {
      try {
    ...
    if (scanType == DesktopScanType.usb) {
  ...
  Future<void> _requestPermissions(DesktopScanType scanType) async {
    if (scanType == DesktopScanType.usb) {
  ...
  Future<void> _scanForDevices() async {
    try {
      if (_selectedScanType == DesktopScanType.usb) {
  ...
  void _stopScanning() {
    ...
    if (_selectedScanType == DesktopScanType.bluetooth)
          ...
          children: [
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap: () => _startScanning(DesktopScanType.usb),
                child: Container(
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 28),
              child: GestureDetector(
                onTap: () => _startScanning(DesktopScanType.bluetooth),
                child: Container(
                  width: double.infinity,
                  decoration: BoxDecoration(
                    color: Colors.white,
