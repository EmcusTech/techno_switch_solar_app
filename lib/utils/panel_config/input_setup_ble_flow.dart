...
enum InputSetupBleFlowMode { download, apply }

Future<void> runInputSetupDownloadFlow({
  ...
  return _runInputSetupBleFlow(
    context: context,
    isMounted: isMounted,
    bleProcess: bleProcess,
    bleController: bleController,
    mode: InputSetupBleFlowMode.download,
    onDownloadComplete: onDownloadComplete,
    showDownloadSuccess: showDownloadSuccess,
  );
}

Future<void> runInputSetupApplyFlow({
  required BuildContext context,
  required bool Function() isMounted,
  required BleProcess bleProcess,
  required BleLogController bleController,
  ...
  return _runInputSetupBleFlow(
    ...
    bleProcess: bleProcess,
    bleController: bleController,
    mode: InputSetupBleFlowMode.apply,
    onApplyComplete: onApplyComplete,
    showApplySuccess: showApplySuccess,
  );
}

Future<void> _runInputSetupBleFlow({
  required BuildContext context,
  required bool Function() isMounted,
  required BleProcess bleProcess,
  required BleLogController bleController,
  required InputSetupBleFlowMode mode,
  ...
  if (mode == InputSetupBleFlowMode.download) {
    bleProcess.isInputSetupFetchDone.value = false;
    bleProcess.isInputSetupFetchCommandActive.value = true;
    unawaited(bleController.startInputSetupFetch());
  } else {
    bleProcess.isInputSetupApplyDone.value = false;
    bleProcess.isInputSetupApplyActive.value = true;
    unawaited(bleController.startInputSetupApply());
  }
  ...
  await showDialog<void>(
    ...
    builder: (dialogContext) {
      return _InputSetupBleProgressDialog(
        bleProcess: bleProcess,
        mode: mode,
        onSucceeded: () async {
          ...
          if (mode == InputSetupBleFlowMode.download) {
            ...
            if (isMounted()) {
              showDownloadSuccess?.call(StringConstants.inputs);
            }
          } else {
            await onApplyComplete?.call();
            if (isMounted()) {
              showApplySuccess?.call(StringConstants.inputs);
            }
          }
        },
        onFailed: () {
...
class _InputSetupBleProgressDialog extends StatefulWidget {
  const _InputSetupBleProgressDialog({
  ...
  final InputSetupBleFlowMode mode;
  final Future<void> Function() onSucceeded;
  final VoidCallback onFailed;

  @override
  State<_InputSetupBleProgressDialog> createState() =>
      _InputSetupBleProgressDialogState();
}

class _InputSetupBleProgressDialogState
    extends State<_InputSetupBleProgressDialog> {
  bool _finished = false;

  @override
  void initState() {
    super.initState();
    widget.bleProcess.isInputSetupFetchDone.addListener(_onFetchDone);
    widget.bleProcess.isInputSetupApplyDone.addListener(_onApplyDone);
    widget.bleProcess.maxOtherPacketsRetriesReached.addListener(_onFailed);
  }

  @override
  void dispose() {
    widget.bleProcess.isInputSetupFetchDone.removeListener(_onFetchDone);
    widget.bleProcess.isInputSetupApplyDone.removeListener(_onApplyDone);
    widget.bleProcess.maxOtherPacketsRetriesReached.removeListener(_onFailed);
    super.dispose();
  }

  void _onFetchDone() {
    if (_finished || widget.mode != InputSetupBleFlowMode.download) return;
    if (!widget.bleProcess.isInputSetupFetchDone.value) return;
    _finishSucceeded();
  }

  void _onApplyDone() {
    if (_finished || widget.mode != InputSetupBleFlowMode.apply) return;
    if (!widget.bleProcess.isInputSetupApplyDone.value) return;
    _finishSucceeded();
  }
  ...
  String _statusText(bool failed, String processDesc) {
    ...
    return widget.mode == InputSetupBleFlowMode.download
        ? StringConstants.downloadInputSetup
        : StringConstants.applyingInputSetup;
  }
