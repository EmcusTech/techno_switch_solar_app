...
class SerialCommunicationService {
  ...
  int _pktTxCnt = 0;
  int _pktRxCnt = 0;
  int _logEvtSearchNumber = 1000; // Start from 1000th log
  ...
  final StreamController<LogModel> _logStreamController = StreamController<LogModel>.broadcast();
  ...
  Future<bool> connectToDevice() async {
    try {
      ...
      for (UsbDevice device in devices) {
        _statusStreamController.add("Trying device: ${device.productName ?? 'Unknown'}");
        ...
        try {
          ...
          await Future.delayed(Duration(milliseconds: 100)); // Give time for device to settle
          ...
          try {
            ...
            _statusStreamController.add("Connected to device: ${device.productName ?? 'USB Device'}");
  ...
  void _reqNwkPkt() {
    ...
    List<int> nwkReqPkt = [
      0xfe, 0x01, 0x00, 0x04, _pktTxCnt & 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  void _reqAccessKey() {
    ...
    List<int> accessKeyReqPkt = [
      0xfe, 0x01, 0x00, 0x01, (_pktTxCnt & 0xFF), 0x00, 0x00, 0x00, 0x00, 0x00, 0x83, 0x00, 0x04, 0x04, 0x31, 0x39, 0x37, 0x34,
  ...
  void _sendDummyPkt() {
    ...
    List<int> framePacket = _frameTheTxCommPkt(
      scriptDest, scriptOrig, PacketType.nrm.value, _pktTxCnt & 0xFF, _pktRxCnt & 0xFF,
      logNw, logNd, logSnd, logMo, logMd, logSk, logCmd, evtLogPayloadData, 5
    );
  ...
  List<int> _frameTheTxCommPkt(int des, int orig, int pkttyp, int txp, int rxp, 
      int nwk, int node, int subnode, int module, int mode, int sock, int cmd, 
      List<int> data, int dataLe) {
  ...
  void _sendData(Uint8List data) async {
    if (_port != null) {
      try {
        ...
        await Future.delayed(Duration(milliseconds: 50)); // Small delay between writes
      } catch (e) {
  ...
  void _onDataReceived(Uint8List data) {
    if (data.length >= 216) {
      ...
      // Handle partial packets or different sized packets
      _statusStreamController.add("Received partial packet: ${data.length} bytes");
    }
  }
    ...
    switch (_processState) {
      ...
      case ProcessState.readEvtLog:
        if (_commFrame.pktTyp == PacketType.nrm.value &&
          ...
          // Don't restart - continue trying to read logs
        }
        break;
  ...
  void _processEventLog(List<int> evtData) {
    ...
    if (timestampDecimal != 0x00) {
      ...
      DateTime eventTime = TimestampConverter.clockTimeFromTimeStamp(timestampDecimal);
