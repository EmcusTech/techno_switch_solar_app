...
/// Debug helpers for inspecting SETUP_EXT_OUT 216-byte apply frames.
abstract final class ExtOutSetupPayloadDebug {
  ...
  static void printApplyFrame(BleManager manager) {
    ...
    final packet = manager.buildExtOutSetupApplyPacket(previewOnly: true);
    debugPrint(
      'SETUP_EXT_OUT APPLY 216-byte frame (preview):\n${formatHexDump(packet)}',
    );
    debugPrint(
      'SETUP_EXT_OUT APPLY one-liner: '
  ...
  static void printFrame(
    List<int> packet, {
    String label = 'SETUP_EXT_OUT APPLY',
  }) {
    if (!kDebugMode) return;
